<script lang="ts">
	...
	function formatDate(date: string) {
		...
		return [
			getDayName(date),
			monthNames[month],
			dateStr.concat(nthNumber(dateTime.getDate())),
			year,
			dateStr
		];
	}
...
<main class="bg-[#27232F] my-4 flex flex-col items-center">
					...
					<ClickOutside on:clickoutside={close}>
						<div class="relative inline-block text-left mb-2">
							<button
								...
								class="text-[#27232F] bg-amber-100 font-mono rounded-full text-base font-bold px-5 py-2.5 text-center inline-flex items-center"
								type="button"
							...
							{#if showDrop}
								...
								<div
									...
									id="dropdownBgHover"
									class="absolute right-0 mt-2 z-10 w-48 bg-[#4b4359] rounded-2xl shadow"
								>
									<ul
										class="p-3 space-y-1 text-sm text-amber-100"
										...
										<li>
											<div class="flex items-center -mb-1 p-2 rounded">
												<input
													...
													type="checkbox"
													value=""
													class="w-4 h-4 text-amber-100 rounded accent-[#27232F]"
													bind:checked={selectAll}
													on:click={toggleAll}
												/>
												<label
													for="checkbox-item-all"
													class="w-full ms-2 text-sm font-mono text-amber-100 rounded"
													>Select All</label
												>
											</div>
										</li>
	...
	{:else}
		<div class="w-screen h-screen flex flex-col items-center justify-center">
			<h1 class="text-lg md:text-3xl text-amber-100 font-mono -mt-10 mb-5 mx-2 text-center">
				Could not find events for this artist ;-;
			</h1>
