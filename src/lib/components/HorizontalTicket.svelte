<script lang="ts">
	...
	function shortenString(inputStr: string) {
		...
		if (
			lastTwoWords.toLocaleLowerCase() === 'presented by' ||
			lastTwoWords.toLocaleLowerCase() === 'at the'
		) {
...
<div
	class="star relative px-10 sepia-[0.4] transition duration-200 hover:scale-[1.02] hover:sepia-0"
>
	<a
		href={link}
		target="_blank"
		...
		class="group my-6 flex flex-row shadow-lg items-center max-w-[840px]"
	>
		<div class="w-60 h-60 relative">
			...
			/>
			<ul
				class="flex space-x-3 text-xs font-mono font-bold px-1 pt-2 -rotate-90 origin-top-left justify-center whitespace-nowrap"
			>
				<li class="text-white/20">ADMIT ONE</li>
				<li class="text-white/50">ADMIT ONE</li>
				<li class="text-white/20">ADMIT ONE</li>
			</ul>
			<p class="absolute bottom-0 right-0 text-white/50 text-sm font-mono font-bold px-3 py-2">
				#{code}
			</p>
		</div>
		...
		<div class="flex bg-[#fffef8] h-60 w-[600px]">
			<div class="w-[430px] text-lg">
				<div
					style="height: unset;"
					class="flex justify-around pt-1.5 items-center font-mono uppercase text-zinc-800 text-center my-3 mx-4 py-1 border-y-[1px] border-zinc-800"
				>
					<p>{date[0]}</p>
					<p>{date[1]} {date[2]}</p>
					<p>{date[3]}</p>
				...
				<p class="mt-5 text-3xl text-zinc-800 uppercase font-mono font-semibold m-2 text-center">
					{artist}
				</p>
				<p class="mt-3 text-lg uppercase text-center font-dosis font-normal mx-2">
					{shortenString(event)}
				</p>
				<p
					class="mt-3 text-lg uppercase text-center font-mono underline underline-offset-8 decoration-zinc-800 decoration-[0.5px] mx-4 line"
				>
					@{shortenString(venue)}
				</p>
			...
			<div class="relative w-[170px] h-60 border-zinc-800/50 border-dashed border-l-2">
				<div class="relative w-[172px] h-60 bg-[#fffef8] box">
					<div class="absolute top-0 inset-0 w-full left-[42px] -rotate-90">
						<ul
							class="w-full flex space-x-3 text-xs font-mono font-bold whitespace-nowrap justify-center"
						>
					...
					<p
						class="pt-2.5 font-mono font-bold text-zinc-800/50 absolute top-1/2 transform -translate-y-4 right-0 -rotate-90"
					>
						#{code}
					</p>
				</div>
	...
	<div
		class="text-2xl text-zinc-800/20 right-12 bottom-1 absolute transition ease-in-out duration-100 hover:text-zinc-800/100"
	>
		<button class="z-50" on:click={onDownload}>
